...
def predict_housing_price(a,b,c,d,e):
    with open('lrm/models/linear_regression.pickle','rb') as f:
...
def housing_price_model_details():
    ...
    X = open('lrm/models/X_col_names_LR.pickle','rb')
    X_names_LR = pickle.load(X)
    
    y_test = open('lrm/models/y_test_LR.pickle','rb')
    y_test_LR = pickle.load(y_test)
    
    X_test = open('lrm/models/X_test_LR.pickle','rb')
